...
import com.soft1010.common.httpapi.annotation.*;
 ...
 *         Description: api动态服务代理
 */
public class ApiServiceInvocationHandler implements InvocationHandler {

    // 日志
    private static final Logger LOGGER = LogManager.getLogger(ApiServiceInvocationHandler.class);
    // http连接池配置
    ...
    private String internInvoke(ApiInvocation apiInvocation) throws Exception {
        ...
        if (null != args) {
            for (int i = 0; i < args.length; i++) {
                ...
                // 原始数据
                else if (ApiRaw.class.isAssignableFrom(parameterTypes[i])) {
                    if (args[i] instanceof String) {
                        rawData = (String) args[i];
                    }
                }
                // 如果是基础类型
                else if (parameterTypes[i].isPrimitive() || ApiObjectUtils.isRawType(parameterTypes[i])) {
                    resolveRawTypeParam(paramAnnotations, i, args, httpParamList, httpHeaderList, jsonParams);
                }
                // 处理复杂类型
                else {
                    resolveComplexTypeParam(args[i], parameterTypes[i], httpParamList, jsonParams, paramAnnotations[i]);
                }
            }
        }
        ...
        HttpUriRequest httpMethod = HttpMethodFactory
                .createHttpMethod(strMethod, apiInvocation.getUrl(), params, httpHeaderList,
                        nameValuePairs, rawData, apiInvocation.getContentType(), jsonParams);
        ...
        try {
            ...
            for (ApiInterceptor apiInterceptor : apiInterceptors) {
                try {
                    apiInterceptor
                            .afterCompletion(httpStatus, apiInvocation.getMethod(), apiInvocation.getUrl(), nameValuePairs,
                                    result, time);
                } catch (Exception e) {
    ...
    private void resolveRawTypeParam(Annotation[][] paramAnnotations, int index, Object[] args,
        ...
        for (Annotation annotation : curParamAnnotations) {
            ...
            if (ApiParam.class.isInstance(annotation)) {
                ...
                // 2 json
                jsonParam.put(apiParam.paramKey(), curParamValue);
     ...
     * 处理复杂类型参数
     */
    private void resolveComplexTypeParam(Object obj, Class parameterType,
                                         List<NameValuePair> nameValuePairList,
                                         Map<String, Object> jsonParams, Annotation[] annotations) {

        for (Annotation annotation : annotations) {
            if (ApiParam.class.isInstance(annotation)) {
                ApiParam apiParam = (ApiParam) annotation;
                jsonParams.put(apiParam.paramKey(), obj);
            }
        }
